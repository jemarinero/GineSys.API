...
namespace GineSys.API.Controllers
{
    ...
    {
        private readonly IParentescosRepository _repo;
        private readonly IMapper _mapper;
        public ParentescosController(IParentescosRepository repo, IMapper mapper)
        {
            this._mapper = mapper;
            this._repo = repo;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]ParentescoToCreateDto param)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);

            var data = _mapper.Map<Parentesco>(param);
            _repo.Add(data);
            await _repo.SaveAll();
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody]ParentescoToUpdateDto param)
        {
            if (id != param.Id)
